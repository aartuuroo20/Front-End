...
const CharacterPage: React.FC<CharacterPageProps> = ({ character }) => {
  return (
    <div className="recuadros">
      ...
      <Link className="todosLinks" href={`/location/${character.location.id}`}> Location: {character.location.name}</Link>
      <p> Episodes: </p>
      <div>
       {character.episode.map((episode) => {
        return (
          <div key={episode.id}>
            <li>
              <Link className="todosLinks" href={`/episode/${episode.id}`}>{episode.name}</Link>
            </li>
          </div>
      )})}
      </div>
